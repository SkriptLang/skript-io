...
""")
@Example("throw an error with message \"oops!\"")
@Example("throw an io error with message \"file broke :(\"")
@Example("throw a null pointer error")
@Since("1.0.0")
public class EffThrow extends Effect {
    ...
    public boolean init(Expression<?> @NotNull [] expressions, int matchedPattern, @NotNull Kleenean kleenean,
        ...
        if (expressions[0] instanceof Literal<?> literal && literal.getSingle() instanceof ErrorInfo<?> info) {
            errorType = info;
        } else {
    ...
    protected void execute(@NotNull Event event) {
        Throwable throwable;
        if (hasMessage) {
            throwable = errorType.create(message.getSingle(event));
        } else {
            throwable = errorType.create(null);
        }
    ...
    public @NotNull String toString(Event event, boolean debug) {
        ...
        return "throw an " + errorType.toString(event, debug);
    }
