...
public class Types {
    ...
    private URI subPath(URI base, URI child) {
        ...
        if (initial.endsWith("/")) {
            while (addendum.startsWith("/")) {
                addendum = addendum.substring(1);
            }
        } else if (initial.endsWith(File.separator)) { // windows patch
            while (addendum.startsWith(File.separator)) {
                addendum = addendum.substring(File.separator.length());
            }
        } else if (!addendum.startsWith("/")) {
            addendum = '/' + addendum;
        }
    ...
    public void registerTypes() {
        Classes.registerClass(new ClassInfo<>(URI.class, "path")
                .user("(path|url)s?").name("Resource Path")
        ...
        Classes.registerClass(new ClassInfo<>(Transaction.class, "transaction")
                .user("transactions?")
                .name("HTTP Transaction")
                .description("Represents an incoming HTTP request or an outgoing HTTP response (e.g. asking for, being " +
                        "asked for, sending, or receiving data). Common request/response features are available here, " +
                        "such as content types and status codes.")
                .examples("the response")
                .since("1.0.0"));
        ...
        Classes.registerClass(new ClassInfo<>(Response.class, "response")
                .user("(?:web)? responses?")
                .name("Web Response")
    ...
    public void registerFileFormats() { // TODO - i dont know :(

    }

    public void registerComparators() {
        Comparators.registerComparator(FileController.class, FileController.class, (first, second) ->
                Relation.get(first.equals(second)));

        Comparators.registerComparator(URI.class, URI.class, (first, second) -> Relation.get(first.equals(second)));

        Comparators.registerComparator(URI.class, FileController.class, (uri, controller) ->
                Relation.get(uri.equals(controller.getPath())));
    }

    public void registerConverters() {
        Converters.registerConverter(FileController.class, URI.class, FileController::getPath);
        Converters.registerConverter(String.class, URI.class, SkriptIO::uri, Converter.NO_LEFT_CHAINING);
    }

    public void loadFormats(SkriptAddon addon, Format<?>... formats) {
        for (Format<?> format : formats) {
            loadFormat(format, addon);
        }
    }
    ...
    void removeEffChange() {
        ...
        syntax.remove(change);
    }

    @SuppressWarnings("unchecked")
    void reAddEffChange() {
        effects.add((SyntaxElementInfo<? extends Effect>) change);
        syntax.add((SyntaxElementInfo<? extends Statement>) change);
        effects = null;
        syntax = null;
        change = null;
    }
    ...
    public void registerErrorTypes(SkriptAddon addon) {
        registerErrorType(new ErrorInfo<>("error", Exception.class, Exception::new, Exception::new)
                .user("error", "exception"), addon);

        registerErrorType(new ErrorInfo<>("ioexception", IOException.class, IOException::new, IOException::new)
                .user("io exception", "io error"), addon);

        registerErrorType(new ErrorInfo<>("nullpointerexception", NullPointerException.class,
